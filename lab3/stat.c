...
void print_histogram(const double v[], int n, int x_start, double y_scale, char mark) {
    int how_many_marks = 0;
    for (int i = x_start; i<13;++i){
        how_many_marks = round(v[i]/y_scale);
        printf("%*d |",2,i);
        for (int j = 0; j<how_many_marks;++j){
            printf("%c",mark);
        }
        printf(" %.3f\n",v[i]);
    }
...
void monty_hall(int n, int *p_switch_wins) {
    int door_chosen, winning_door;
    for (int i = 0;i<n;++i){
        door_chosen = 1 + rand()%3;
        winning_door = 1 + rand()%3;
        if (door_chosen!=winning_door){
            (*p_switch_wins)++;
        }
    }
}
...
int main(void) {
    ...
    double arithmetic_mean = 0, variance = 0, probability, d_vector[100];
    scanf("%d", &to_do);
    scanf("%d",&seed);
    scanf("%d",&n);
    srand((unsigned)seed);
